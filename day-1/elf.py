...
is a special type of star fruit that only grows deep in the jungle.
...
approach land, the Elves begin taking inventory of their supplies.
...
being carried by the Elf carrying the most Calories. In the example above,
...
with open("../inputs/day-1/elf.txt", "r") as _file:
    blocks = _file.read().split("\n\n")

sums = [sum(map(int, block.split())) for block in blocks]
ranked = sorted(sums)
print(ranked[-1])
print(sum(ranked[-3:]))
